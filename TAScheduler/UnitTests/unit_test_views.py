...
class ManageSectionViewTests(TestCase):
    ...
    def test_manage_section_view_invalid_section_data(self):
        """Test if invalid section data is rejected (e.g., missing fields)."""
        response = self.client.post(reverse('manage_section'), {
            'section_id': 4,
            'course': self.course.id,
            'location': '',
            'meeting_time': 'Mon-Wed 3:00 PM - 4:30 PM'
        })
        self.assertFormError(response, 'form', 'location', 'This field is required.')

    def test_manage_section_view_edit_invalid_data(self):
        """Test if editing a section with invalid data is rejected."""
        response = self.client.post(reverse('manage_section_edit', args=[self.section_lab.id]), {
            'section_id': self.section_lab.section_id,
            'course': self.course.id,
            'location': '',
    ...
    def test_manage_section_view_create_section_without_required_fields(self):
        """Test if trying to create a section without all required fields returns an error."""
        response = self.client.post(reverse('manage_section'), {
            'section_id': 3,
            'course': self.course.id,
            'location': '',
